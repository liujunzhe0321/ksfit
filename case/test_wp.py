...
class walkingPadClass(unittest.TestCase):
    '''wp国内接口'''
    wp_http = None
    wp_cookies=None
    ...
    def setUp(self):
        self.wp_http = ksfitUtilClass().readIni("wpinformation","wp_http")
        self.wp_cookies=ksfitLoginCommonClass().wpDomesticLogin()
    ...
    def test_wpGetlist(self):
        ...
        httpUrl=self.wp_http+"/user/api/v2/record"
        response = requests.get(httpUrl,data=json.dumps(data),cookies=self.wp_cookies)
